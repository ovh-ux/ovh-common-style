module.exports = function (grunt) {
    ...
    grunt.initConfig({
            ...
            dist: {
                files: {
                    "<%= builddir %>/<%= name %>.js": [
                        ...
                        "<%= srcdir %>/**/*.js",
                        "!<%= srcdir %>/**/*.spec.js",
                        "<%= builddir %>/spinner/tpls.js",
                        "<%= builddir %>/spinner/add-tpls.js"
                    ]
                }
            },
        ...
        loaderToHtml: {
            args: {
                src: "<%= srcdir %>/spinner/gif",
                tmp: "<%= builddir %>/spinner/gifTmp",
                dest: "<%= builddir %>/spinner/html"
            }
        },
        ngtemplates: {
            "ovh-spinner": {
                src: "<%= builddir %>/spinner/html/*.html",
                dest: "<%= builddir %>/spinner/tpls.js",
                options: {
                    module: "ovh-common-style",
                    url: function (url) {
                        var split = url.split("/");
                        var fileName = split.length ? split[split.length - 1] : "";
                        return fileName.replace("load-", "");
                    },
                    prefix: "spinner/"
                }
            },
            additional: {
                src: "<%= srcdir %>/spinner/html/*.html",
                dest: "<%= builddir %>/spinner/add-tpls.js",
                options: {
                    module: "ovh-common-style",
                    url: function (url) {
                        var split = url.split("/");
